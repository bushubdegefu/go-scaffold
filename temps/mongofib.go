...
func MongoFiberFrame() {
	//  this is creating manger file inside the manager folder
	// ############################################################
	devf_tmpl, err := template.New("RenderData").Parse(mongodevfTemplate)
	if err != nil {
		panic(err)
	}
	...
	// #################################################
	prod_tmpl, err := template.New("RenderData").Parse(mongoprodTemplate)
	if err != nil {
		fmt.Println(err)
		panic(err)
	}
