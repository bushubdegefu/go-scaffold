...
func TestFrameFiber() {
	...
	test_tmpl, err := template.New("data").Parse(testTemplateFiber)
	if err != nil {
		panic(err)
	}
	...
	// ###################################################################
	test_app_tmpl, err := template.New("data").Parse(testAppTemplate)
	if err != nil {
		panic(err)
	}
	//
	...
	err = test_app_tmpl.Execute(test_app_file, data)
	if err != nil {
		panic(err)
	}
