...
func CommonGraphQLFrame() {

	// ############################################################
	common_tmpl, err := template.New("RenderData").Parse(commonGraphQLTemplate)
	if err != nil {
		panic(err)
	}
...
func PaginationPureModelFilterOneToMany(db *gorm.DB, queryModel interface{}, responseObjectModel interface{}, otm_string string, parentID uint,
	...
	go func(comm <-chan int64) {
		...
		if tracer != nil {
			...
			db.Select("*").Model(&queryModel).Where(otm_string, parentID).Count(&local_counter)
		}
		count_channel <- local_counter
	...
	go func(comm <-chan string) {
		if page == 1 {
			if tracer != nil {
				...
				db.Model(&queryModel).Where(otm_string, parentID).Order("id asc").Limit(int(size)).Offset(0).Find(&responseObjectModel)
			}
			response_page = 1
		} else {
			if tracer != nil {
				...
				db.Model(&queryModel).Where(otm_string, parentID).Order("id asc").Limit(int(size)).Offset(int(offset)).Find(&responseObjectModel)
			}
			// response_channel <- loc_resp
			response_page = int64(page)
		}
...
func PaginationPureModelFilterManyToMany(db *gorm.DB, queryModel interface{}, responseObjectModel interface{}, parentModel interface{}, parentID uint,
	page uint, size uint, tracer context.Context) (ResponsePagination, interface{}, error) {
	if size > 50 {
		size = 50
	}
	...
	go func(comm <-chan int64) {
		var local_counter int64
		if tracer != nil {
			// Model(&language).Related(&users,  "Users")
			db.WithContext(tracer).Select("*").Model(&parentModel).Where("id = ? ", parentID).Count(&local_counter)
		} else {
			db.Select("*").Model(&queryModel).Count(&local_counter)
		}
		count_channel <- local_counter
	...
	go func(comm <-chan string) {
		if page == 1 {
			if tracer != nil {
				db.WithContext(tracer).Model(&parentModel).Where("id = ? ", parentID).Order("id asc").Limit(int(size)).Offset(0)
			} else {
				db.Model(&queryModel).Order("id asc").Limit(int(size)).Offset(0).Find(&responseObjectModel)
			}
			response_page = 1
